...
<div id="page-content" style="min-height: 200px; " class="home-page">
    ...
    <li>
        ...
        <a class="name" href="<?php echo site_url('my_city/visit')?>/<%:=data[i].focus_user_id%>"><%:=data[i].nickname%></a>
        <span class="follows">粉丝：<span><%:=data[i].fans_lists%></span></span>
        <span>关注：<span><%:=data[i].focus_lists%></span></span>
        <% if(data[i].is_focused) {%>
        <a class="btn btn-link add_focus focused_btn" data-focus-id="<%:=data[i].focus_user_id%>">
            <i class="fa fa-check" aria-hidden="true"></i>
            取消关注
        </a>
        <% } else if(data[i].is_me) {%>
        <span class="btn btn-link add_focus focus">
            <i class="fa" aria-hidden="true"></i>
            我
        </span>
        <% } else {%>
        <a class="btn btn-link add_focus focus_btn" data-focus-id="<%:=data[i].focus_user_id%>">
