 ...
 */
app.controller('commonclassCtrl', ['$scope', '_jiyin', 'dataToURL', '$stateParams', '$state', function ($scope, _jiyin, dataToURL, $stateParams, $state) {
     ...
     * 增加
     */
    /*$scope.addCommonclassList = function () {
        $scope.title = '增加数据';
        _jiyin.modal({
    ...
    $scope.addList = function (infoList) {
        infoList.is_point = 0;
     ...
     * 编辑
     *!/
    $scope.editCommonclassList = function (data) {
        $scope.title = '编辑数据';
        _jiyin.modal({
            tempUrl : '/source/admin/tpl/modal/modal-commoClass.html',
            tempCtrl : 'modalCommoClassCtrl',
            ...
            params : {
                title: $scope.title,
                commonclassList: angular.copy(data),
                ael: 'edit',
                isPoint: false
            }
        });
    };
    $scope.updateList = function (infoList) {
