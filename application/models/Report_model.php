...
Class Report_model extends CI_Model{
    ...
    public function add($order_commodity_id, $number, $name, $age, $gender, $phone, $identity_card, $attachment_id) {
        ...
        if (intval($order_commodity_id) < 1 || empty($number) || empty($name) || empty($age) || intval($gender) > 1 || intval($gender) < 0 || empty($phone) || empty($identity_card) || intval($attachment_id) < 1) {
            $result['msg'] = '参数不正确';
            return $result;
        }
